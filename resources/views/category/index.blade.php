...
<h4> Inventory </h4>
    ...
    <div class="row my-1 g-0">
        ...
        @csrf
                ...
                <td><img src="{{ asset('storage/products/'.$cat_data->image) }}" class="img-fluid" width ="90"> </td>
                <td>{{ $cat_data->name }}</td>
                <td>{{ $cat_data->description }}</td>
                <td> {{ $cat_data->section }}</td>
                <td> {{ $cat_data->qty }}</td>
                <td>Php {{ $cat_data->price }}</td>
                <td><a type="button" href="/category/{{ $cat_data->id }}" class="btn btn-sm btn-primary mb-3">View</a>
                <button class="btn btn-sm btn-danger mb-3" type="submit">Delete </a></td>
                </tr>
                @endforeach
            </tbody>
        </table>
